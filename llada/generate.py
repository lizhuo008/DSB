...
@ torch.no_grad()
def generate_dsb(model, prompt, steps=128, gen_length=128, block_length=128, temperature=0.,
...
@ torch.no_grad()
def generate_dsb_cache(model, prompt, steps=128, gen_length=128, block_length=128, temperature=0.,
    ...
    while num_unmask < gen_length:
        ...
        num_acc = cur_transfer_index.sum(dim=-1, keepdim=True).item()
        ...
        while True:
            ...
            cur_idx = cur_mask_blk.to(torch.long).argmax(dim=1).item() if has_mask else cur_mask_blk.shape[1]
            s = s + cur_idx 
